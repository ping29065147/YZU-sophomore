...
public:
	...
	vector<string> load(string input, int line)
	{
		reset();
		...
		{
			label[input.substr(0, input.find(':'))] = to_string(line * 4);
			ss >> tmp;
		}
		ss >> tmp;
		V.emplace_back(tmp);
	...
	// convert risc-v code to machine code
	void convert()
	{
		...
		else if (sb_type.find(V[0]) != sb_type.end())
		{
			...
			if (label.find(V[3]) != label.end()) imm = binary(label[V[3]], 13);
			result = imm.substr(0, 1) + imm.substr(2, 6) + binary(V[2], 5) + binary(V[1], 5) + sb_type[V[0]]
